describe("Pokedex testing", () => {
  ...
  beforeEach(() => {
    cy.visit(localhost);
    cy.intercept("GET", firstPage, { fixture: "pokedexFirstPage" }).as("getFirstPage");
    cy.intercept("GET", pokemonListURL, { fixture: "pokemonList" }).as("getAllPokemons");
  });
  ...
  it("Uses the pokemon search box", () => {
    cy.intercept("GET", blastoiseURL, { fixture: "blastoise" }).as("getBlastoise");
    cy.intercept("GET", blastoiseSpeciesURL, { fixture: "blastoiseSpecies" }).as("getBlastoiseSpecies");

    cy.get("[data-cy='pokemon-search-box']").click();
    cy.get("[data-cy='pokemon-search-box-list']").children().should("have.length", 1292);
    ...
    cy.get("[data-cy='pokemon-search-box-list']").should("not.be.visible");
  ...
  it("Uses paginator", () => {
    cy.intercept("GET", secondPage, { fixture: "pokedexSecondPage" }).as("getSecondPage");
    cy.intercept("GET", lastPage, { fixture: "pokedexLastPage" }).as("getLastPage");

    cy.get("[data-cy='page-2']").click();

    cy.get("[data-cy='previous-page-button']").should("have.not.class", "disabled");
    cy.get("[data-cy='pokemon-cards-container']").children().should("have.length", 20);
    ...
    cy.get("[data-cy='pokemon-cards-container']").children().should("have.length", 12);
  });
});
